...
@section('title', basename($file->file_path) . ' - Coverage')

@section('content')
    <div class="mb-6">
        <a href="{{ route('dashboard.branch', [$repository, $branch]) }}" class="text-blue-600 hover:underline text-sm">&larr; {{ $branch }}</a>
        <h1 class="text-xl font-bold text-gray-900 mt-2 font-mono">{{ $file->file_path }}</h1>
        <div class="flex items-center space-x-4 mt-1">
            ...
            <span class="text-sm text-gray-500">{{ $file->covered_lines }}/{{ $file->total_lines }} lines</span>
        </div>
    </div>
